...
const ChangeProjectsStatusConstruction = () => {
    ...
    const handleDelete = (id, status) => {
        Swal.fire({
            title: "Are you sure?",
            text: `You Are Change The Status`,
            icon: "warning",
                    ...
                    projectsData.map((project, index) =>
                        <div key={index}>
                            <div className='px-3 border-2 border-[#FFBF00] rounded-xl py-2 shadow-md shadow-[#FFBF00] w-full flex items-center justify-between gap-7 cursor-pointer'>
                                ...
                                <button onClick={() => { handleDelete(project._id, project?.status ? false : true) }} className='border-2 border-[#FFBF00] px-8 py-2 rounded-full hover:text-[#FFBF00] hover:border-white duration-200'>{project.status ? 'Complete' : 'Running'}</button>
                            </div>
                        </div>
                    )
                }
...
export default ChangeProjectsStatusConstruction;